...
protected:
	...
	void DF_constantDensitySolverFirst();

	void DF_constantDensitySolverSecond();

	void DF_divergenceFreeSolver();

	void ii_forceExceptPressure();

	void DF_forceExceptPressure();

	void streamCorrection();
	...
	// DFSPH version of ii_fluidPartForceExceptPressure_fsame()
	inline void DF_fluidPartForceExceptPressure_fsame(
		FluidPart& fa, const FluidPart& fb, const real_t& dis,
		const real_t& fm0, const real_t& alpha, const real_t& gamma);
	...
	inline void DF_fluidBoundExceptPressure(
